...
public class ProductoImpl extends Conexion implements ICRUD<Producto> {
    ...
    public List<Producto> listar(int estado) throws Exception {
        ...
        switch (estado) {
            case 1:
                sql = "SELECT * FROM PRODUCTO WHERE ESTPRO ='A' ORDER BY CODPRO DESC";
                break;
            case 2:
                sql = "SELECT * FROM PRODUCTO WHERE ESTPRO ='I' ORDER BY CODPRO DESC";
                break;
            case 3:
                sql = "select * from PRODUCTO ORDER BY CODPRO DESC";
                break;
        }
        try {
            this.conectar();
